/*
    ...
    and all of the parameters that change during the duration of a test are saved in the csv file.  Some examples of the data that is recorded are the robot2's position, the current goal location, and the
    current distance to the goal.  The logs are also sorted out into folders that are named using the current level, robot2, and the initial timestamp of the log.  The logs can be found on the resources/logs
    folder.
*/
...
public:
    Logger(ros::NodeHandle n)
    {
        ...
        std::string file_path_csv = file_path + "/robot2_sim_log.csv";
        std::string file_path_txt = file_path + "/robot2_config.txt";
