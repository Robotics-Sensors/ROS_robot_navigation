/*
    ...
    and all of the parameters that change during the duration of a test are saved in the csv file.  Some examples of the data that is recorded are the obstacle_bot 's position, the current goal location, and the
    current distance to the goal.  The logs are also sorted out into folders that are named using the current level, obstacle_bot, and the initial timestamp of the log.  The logs can be found on the resources/logs
    folder.
*/
...
//SimLog is a custom message type defined in robot_navigation/msg/SimLog.msg
#include <robot_navigation/SimLog.h>
...
public:
    Logger(ros::NodeHandle n)
    {
        ...
        std::string file_path_csv = file_path + "/obstacle_bot_sim_log.csv";
        std::string file_path_txt = file_path + "/obstacle_bot_config.txt";
    ...
    void add_goal(robot_navigation::Goal goal)
    {
        if (!navigating && test_active)
        {
            //Save goal pos
...
void goal_callback(const robot_navigation::Goal::ConstPtr &goal)
{
