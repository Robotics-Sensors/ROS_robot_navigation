...
private:
  std::vector<obstacle_bot_navigation::Goal> obstacle_bot_goal_array;
  ...
  obstacle_bot_goal_publisher(ros::NodeHandle node_handle) {
    ...
    while (std::getline(csvFile, csvLine)){
      if (!csvLine.empty()){
        ...
        if (obstacle_bot_goal_x_position == 0.0 && obstacle_bot_goal_y_position == 0.0 && obstacle_bot_goal_z_rotation == 0.0){
    ...
    obstacle_bot_goal_pub = node_handle.advertise<obstacle_bot_navigation::Goal>("goal", 1, true);
    counter = 1;
  }
