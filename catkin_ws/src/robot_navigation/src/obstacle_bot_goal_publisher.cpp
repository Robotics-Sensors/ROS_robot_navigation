  ...
  robot_goal_publisher(ros::NodeHandle node_handle) {
    ...
    while (std::getline(csvFile, csvLine)){
      if (!csvLine.empty()){
        ...
        if (robot_goal_x_position == 0.0 && robot_goal_y_position == 0.0 && robot_goal_z_rotation == 0.0){
...
int main(int argc, char **argv){
  ...
  /* robot_goal_publisher goal_handler_obj = new robot_goal_publisher(node_handle); */
