...
echo
echo -----------------------------------------------------------
roslaunch robot_navigation "$level".launch sim:="$sim" gui:="$gui" navigate:=true robot:="$robot" 3d:="$threeD" obstacle_bot:="$obstacle_bot" &
pid1=$!
...
echo
echo -----------------------------------------------------------
#cd to the logs folder inside of resources in the robot_navigation package
roscd robot_navigation
cd resources/logs
...
roslaunch robot_navigation geotiff_writer.launch map_dir:=$ROS_WORKSPACE/src/robot_navigation/resources/logs/$name/geotiff_maps &
pid2=$!
...
echo
echo -----------------------------------------------------------
roslaunch robot_navigation start_test.launch iterations:="$iterations" clear_costmaps:="$clear_costmaps"
pid5=$!
